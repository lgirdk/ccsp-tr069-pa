...
static void updateInitalContact (void)
{
    ...
    if ((!lastContactUrl) ||
       (!AnscEqualString(objectInfo[ManagementServerID].parameters[ManagementServerURLID].value, lastContactUrl, TRUE) && lastSavedAcsURLFromDHCP[0] != '\0' && !AnscEqualString(lastSavedAcsURLFromDHCP, lastContactUrl, TRUE)))
    {
        bEnabled = TRUE;
    }
#else
