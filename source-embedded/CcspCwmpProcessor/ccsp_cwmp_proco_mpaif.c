    ...
    if ( TRUE )
    {
        ...
        for ( j = 0; j < nNumFCs; j ++ )
        {
            ...
            if ( flag_pInvalidParam == FALSE )// Remaining SSID passwords and for TR069PSWDCTRLFLAG=true
            {
				...
				if(AnscEqualString(pInvalidParam,"Device.DHCPv4.Server.Pool.1.X_LGI-COM_LanAllowedSubnetTable.1.LanAllowedSubnetIP",TRUE) || \
				   AnscEqualString(pInvalidParam,"Device.DHCPv4.Server.Pool.1.X_LGI-COM_LanAllowedSubnetTable.1.LanAllowedSubnetMask",TRUE) || \
				   AnscEqualString(pInvalidParam,"Device.RootDataModelVersion",TRUE))
				{
					nResult=CCSP_CWMP_CPE_CWMP_FaultCode_notWritable;
				}
            }
        ...
        if(bRadioRestartEn)
        {
            ...
            for(x =0; x<= noOfParam; x++)
            {
                ...
                else if(!strncmp(ParamName[x],"Device.WiFi.Radio.2.",20))
                {
                    ...
                    else if (strncmp(ParamName[x], "Device.WiFi.X_LGI-COM_ATM.Radio.1.", 34) == 0)
                    {
                        bRestartRadio1 = TRUE;
                    }
                    else if (strncmp(ParamName[x], "Device.WiFi.X_LGI-COM_ATM.Radio.2.", 34) == 0)
                    {
                        bRestartRadio2 = TRUE;
                    }
                }
